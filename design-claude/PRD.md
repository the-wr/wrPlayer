...
If the inbox is empty, the picker is **skipped** — the mode is entered directly with nothing playing and the empty-inbox state is shown (see §5.4); there is nothing to order. Otherwise a Sort Order picker (bottom sheet) is shown before entering the mode. The picker pre-selects the last used order. Options: Newest First / Random / Closest to Threshold. Confirming the order begins playback of the first inbox track. Dismissing the sheet cancels the mode switch and returns to Play Mode.
...
- **Track ends** → advances automatically, committing the pending +1 if one is selected. Manually scrubbing the progress bar to the end counts as the track ending and triggers this same advance-and-commit.
...
- Runs on a background thread concurrently with playback; does not block or interrupt the listening experience. If the track advances (skip/next/vote) before detection completes, detection still **finishes in the background for that track** and the result is stored. If detection is interrupted before it stores a result (e.g. the app is killed mid-detection), `bpm_detected` stays null and detection is **re-triggered on the track's next play**.
